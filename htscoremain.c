...
// Main, r�cup�re les param�tres et appelle le robot
HTSEXT_API int hts_main2(int argc, char **argv, httrackp *opt) {
  char** x_argv=NULL;     // Patch pour argv et argc: en cas de r�cup�ration de ligne de commande
  char* x_argvblk=NULL;   // (reprise ou update)
  int   x_ptr=0;          // offset
  //
  int argv_url=-1;           // ==0 : utiliser cache et doit.log
  char* argv_firsturl=NULL;  // utilis� pour nommage par d�faut
  char* url = NULL;          // URLS s�par�es par un espace
  int   url_sz = 65535;
  //char url[65536];         // URLS s�par�es par un espace
  // the parametres
  int httrack_logmode=3;   // ONE log file
  int recuperer=0;         // r�cup�rer un plantage (n'arrive jamais, � supprimer)
#ifndef _WIN32
#ifndef HTS_DO_NOT_USE_UID
  ...
  ensureUrlCapacity(url, url_sz, 65536);
	...
	// Create options
	_DEBUG_HEAD=0;            // pas de debuggage en t�tes
  ...
  /* Create new argc/argv, replace alias, count URLs, treat -h, -q, -i */
  {
    ...
    na=1;             /* commencer apr�s nom_prg */
    while(na<argc) {
      int result=1;
      tmp_argv[0][0]=tmp_argv[1][0]='\0';

      /* V�rifier argv[] non vide */
      if (strnotempty(argv[na])) {
        
        /* V�rifier Commande (alias) */
  ...
  if ( fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-cache/doit.log")) && (argv_url<=0) ) {
    ...
    if (fp) {
      int insert_after=1;     /* ins�rer apr�s nom au d�but */
      //
  ...
  // permet de lancer httrack sans a avoir � se rappeler de syntaxes comme p0C0I0Qc32 ..
#if DEBUG_STEPS
  ...
  */
  ...
  // Ici on ajoute les arguments qui ont �t� appel�s avant au cas o� on r�cup�re une session
  // Exemple: httrack www.truc.fr -L0 puis ^C puis httrack sans URL : ajouter URL pr�c�dente
  /*
  if (argv_url==0) {
    //if ((fexist(fconcat(StringBuff(opt->path_log),"hts-cache/new.dat"))) && (fexist(fconcat(StringBuff(opt->path_log),"hts-cache/new.ndx")))) {  // il existe d�ja un cache pr�c�dent.. renommer
    if (fexist(fconcat(StringBuff(opt->path_log),"hts-cache/doit.log"))) {    // un cache est pr�sent
  ...
  */
  ...
  if (argv_url==0) {
    ...
    if (
      ...
      if (fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-cache/doit.log"))) {    // un cache est pr�sent
        if (x_argvblk!=NULL) {
          int m;        
          // �tablir mode - mode cache: 1 (cache valide) 2 (cache � v�rifier)
    ...
    } else {    // aucune URL d�finie et pas de cache
      if (opt->quiet) {
        help(argv[0],!opt->quiet);
        htsmain_free();
        return -1;
    ...
    if (fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-in_progress.lock"))) {  // fichier lock?
      ...
      //char s[32];
      opt->cache=2;  // cache vient apr�s test de validit�
      if (opt->quiet==0) {
        if (
          ( fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-cache/new.zip")) )
          ||
      ...
      if (cmdl_opt(argv[na])) { // option
        ...
        while(*com) {
          ...
          case 'h': 
            help(argv[0],0); 
            htsmain_free();
            return 0;   // d�ja fait normalement
            //
          case 'g':    // r�cup�rer un (ou plusieurs) fichiers isol�s
            ...
            httrack_logmode=1;            // erreurs � l'�cran
            opt->savename_type=1003;   // mettre dans le r�pertoire courant
...
*/
            //
            // note: les tests opt->depth sont pour �viter de faire
            ...
            if (strcmp(argv[na],"-N")==0) {    // Tout seul
              if ((na+1>=argc) || (argv[na+1][0]=='-')) {  // erreur
                ...
                if (StringLength(opt->savename_userdef) > 0)
                  ...
                  opt->savename_type = 0;    // -N "" : par d�faut
              }
            } else {
          ...
          case 'Z': opt->debug=2; break;  // GROS debug
            //
          case '&': case '%': {    // deuxi�me jeu d'options
            com++;
            switch(*com) {
            ...
            case 'n': opt->norecatch=1; if (*(com+1)=='0') { opt->norecatch=0; com++; } break;   // ne pas reprendre fichiers effac�s localement
          ...
          case '#':  { // non document�
                    ...
                    while ( a != NULL ) {
                      ...
                      if (a) {
                        ...
                        if (!hasFilter
                          ...
                          r = cache_read_ro(opt, &cache, adr, fil, "", NULL);    // lire entr�e cache + data
            ...
            case 'K': opt->keyboard=1; break;  // v�rifier stdin
              //
          ...
          case 'P':    // proxy
            if ((na+1>=argc) || (argv[na+1][0]=='-')) {
              ...
              na++;
              opt->proxy.active=1;
              // Rechercher MAIS en partant de la fin � cause de user:pass@proxy:port
              a = argv[na] + strlen(argv[na]) -1;
              // a=strstr(argv[na],":");  // port
              while( (a > argv[na]) && (*a != ':') && (*a != '@') ) a--;
              if (*a == ':') {  // un port est pr�sent, <proxy>:port
          ...
          case 'F':    // user-agent field
            if ((na+1>=argc) || (argv[na+1][0]=='-')) {
              ...
              if (StringNotEmpty(opt->user_agent))
                ...
                opt->user_agent_send=0;    // -F "" d�sactive l'option
            }
            break;
            //
          case 'V':    // execute command
            if ((na+1>=argc) || (argv[na+1][0]=='-')) {
              ...
              if (StringNotEmpty(opt->sys_com))
                ...
                opt->sys_com_exec=0;    // -V "" d�sactive l'option
            }
            break;
            //
          default: {
		...
		if (urlSize < HTS_URLMAXSIZE) {
			...
			if (strnotempty(url)) strcatbuff(url," ");  // espace de s�paration
  ...
  // on utilise le cache..
  // en cas de pr�sence des deux versions, garder la version la plus avanc�e,
    ...
    if (fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-in_progress.lock"))) {   // problemes..
      if ( fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-cache/new.dat")) ) { 
        ...
        if (fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-cache/old.dat")) && fexist(fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),"hts-cache/old.ndx"))) {
          ...
          // ce cas est peut �tre une erreur ou un crash d'un miroir ancien, prendre
  ...
  
  {
    ...
    // un petit lock-file pour indiquer un miroir en cours, ainsi qu'un �ventuel fichier log
    {
      FILE* fp=NULL;
      //int n=0;
      char t[256];
      ...
      // v�rifier existence de la structure
      ...
      if (opt->log)     {
        ...
        for(i=0;i<argc;i++) {
          ...
          else    // entre "" (si espace(s) et pas d�ja de ")
            fprintf(opt->log,"\"%s\" ",argv[i]);
        }
        fprintf(opt->log,")"LF);
        fprintf(opt->log,LF);
...
int check_path(String* s, char* defaultname) {
  ...
  // ending /
  if (StringNotEmpty(*s) && StringRight(*s, 1) != '/')    // ajouter slash � la fin
    StringCat(*s, "/");

  return return_value;
}

// d�termine si l'argument est une option
int cmdl_opt(char* s) {
  if (s[0]=='-') {  // c'est peut �tre une option
