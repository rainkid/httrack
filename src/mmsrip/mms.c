                   ...
                   const int orig )
{
  ...
  if ( orig == MMS_SERVER )
    fprintf ( stddebug, " command from server (%ld bytes)\n", length );
  else
    fprintf ( stddebug, " command from client (%ld bytes)\n", length );
      ...
      if ( ( guid_1 == 0x6553200cc000e48eULL ) && ( guid_2 == 0x11cfa9478cabdca1ULL ) )
        {
          ...
          if ( mms->stddebug != NULL )
            {
              fprintf ( mms->stddebug, "File Properties Object (%ld bytes)\n", length );
              fprintf ( mms->stddebug, " -> broadcast bit: %d\n", mms_get_32(header,i+64)&1 );
              fprintf ( mms->stddebug, " -> min packet length: %ld\n", packet_length );
      ...
      else if  ( ( guid_1 == 0x6553200cc000e68eULL ) && ( guid_2 == 0x11cfa9b7b7dc0791ULL ) )
        {
          ...
          if ( mms->stddebug != NULL )
            {
              fprintf ( mms->stddebug, "Stream Object (%ld bytes)\n", length );
