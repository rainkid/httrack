...
/* ne pas d�tcter de liens dedans */
...
/* d�tection de mini-code javascript */
...
/* tags o� l'URL doit �tre r��crite mais non captur�e */
const char* hts_detectandleave[] = {
  "action",
  ""
};

/* ne pas renommer les types renvoy�s (souvent types inconnus) */
...
// conversion �ventuelle / vers antislash
#ifdef _WIN32
...
// R�cup�ration d'un fichier http sur le net.
// Renvoie une adresse sur le bloc de m�moire, ou bien
// NULL si un retour.msgeur (buffer retour.msg) est survenue. 
//
// Une adresse de structure htsmsg peut �tre transmise pour
// suivre l'�volution du chargement si le process a �t� lanc� 
// en background
...
// ouvre une liaison http, envoie une requ�te GET et r�ceptionne le header
// retour: socket
...
// ouverture d'une liaison http, envoi d'une requ�te
...
int http_xfopen(httrackp *opt,int mode,int treat,int waitconnect,char* xsend,char* adr,char* fil,htsblk* retour) {
  ...
  // ouvrir liaison
#if HDEBUG
  printf("Cr�ation d'une socket sur %s\n",adr);
#endif
  ...
  if (retour) {
    if ( (!(retour->req.proxy.active)) 
      ...
      soc=newhttp(opt, retour->req.proxy.name, retour,retour->req.proxy.port, waitconnect);  // ouvrir sur le proxy � la place
    }
  } else {
  ...
  if (soc!=INVALID_SOCKET) {    
    ...
    rcvd[0]='\0';
#if HDEBUG
    printf("Ok, connexion r�ussie, id=%d\n",soc);
#endif
    ...
    if (soc!=INVALID_SOCKET) {
...
#if HDEBUG
      printf("Attente de la r�ponse:\n");
#endif
      
      // si GET (r�ception d'un fichier), r�ceptionner en-t�te d'abord,
      // et ensuite le corps
      // si POST on ne r�ceptionne rien du tout, c'est apr�s que l'on fera
      // une r�ception standard pour r�cup�rer l'en t�te
      if ((treat) && (waitconnect)) {  // traiter (attendre!) en-t�te        
        // R�ception de la status line et de l'en-t�te (norme RFC1945)
        
        // status-line � r�cup�rer
        finput(soc,rcvd,1024);
        if (strnotempty(rcvd)==0)
          finput(soc,rcvd,1024);    // "certains serveurs bugg�s envoient un \n au d�but" (RFC)
      ...
      } else { // si GET, on recevra l'en t�te APRES
        //rcvsize=-1;    // on ne connait pas la taille de l'en-t�te
        if (retour)
          retour->totalsize=-1;
      }
...
int http_sendhead(httrackp *opt,t_cookie* cookie,int mode,char* xsend,char* adr,char* fil,char* referer_adr,char* referer_fil,htsblk* retour) {
  ...
  // possibilit� non document�e: >post: et >postfile:
  // si pr�sence d'un tag >post: alors executer un POST
  // exemple: http://www.someweb.com/test.cgi?foo>post:posteddata=10&foo=5
  // si pr�sence d'un tag >postfile: alors envoyer en t�te brut contenu dans le fichier en question
      ...
      if (mode==0) {      // GET!
        ...
        if (fp) {
          ...
          if (sscanf(line,"%s %s %s",method,url,protocol) == 3) {
            ...
            if(fread(buff+strlen(buff),8000-strlen(buff),1,fp)){
            }
          }
          fclose(fp);
        }
  ...
  if (strnotempty(buff)==0) {    // PAS POSTFILE
    ...
    // si on g�re un proxy, il faut une Absolute URI: on ajoute avant http://www.adr.dom
    ...
    // NOM DU FICHIER
    // on slash doit �tre pr�sent en d�but, sinon attention aux bad request! (400)
    if (*fil!='/') strcatbuff(buff,"/");
    {
      ...
      strcatbuff(buff,tempo);       // avec �chappement
    }
    ...
    /* supplemental data */
    if (xsend) strcatbuff(buff,xsend);    // �ventuelles autres lignes
    ...
    if (cookie) {
      ...
      if (cook) {                           // on a envoy� un (ou plusieurs) cookie?
        strcatbuff(buff,H_CRLF);
#if DEBUG_COOK
        printf("Header:\n%s\n",buff);
#endif
    ...
    // g�rer le keep-alive (garder socket)
  ...
  //if (sendc(retour->soc,buff) != strlen(buff)) {  // ERREUR, socket rompue?...
    deletesoc_r(retour);  // fermer tout de m�me
...
// traiter 1ere ligne d'en t�te
void treatfirstline(htsblk* retour,char* rcvd) {
  char* a=rcvd;
  // exemple:
  // HTTP/1.0 200 OK
  if (*a) {
    // note: certains serveurs bugg�s renvoient HTTP/1.0\n200 OK ou " HTTP/1.0 200 OK"
    while ((*a==' ') || (*a==10) || (*a==13) || (*a==9)) a++;      // �purer espaces au d�but
    if (strfield(a, "HTTP/")) {
      // sauter HTTP/1.x
      while ((*a!=' ') && (*a!='\0') && (*a!=10) && (*a!=13) && (*a!=9)) a++;   
      if (*a != '\0') {
        while ((*a==' ') || (*a==10) || (*a==13) || (*a==9)) a++;      // �purer espaces
        if ((*a>='0') && (*a<='9')) {
...
void treathead(t_cookie* cookie,char* adr,char* fil,htsblk* retour,char* rcvd) {
  ...
  // else if ((p=strfield(rcvd,"Transfer-Encoding: chunked"))!=0) {  // chunk!
...
#if HDEBUG
      printf("ok, Transfer-Encoding: d�tect�\n");
#endif
    }
  }
    ...
    if (retour) {
      char tempo[1100];
      // �viter les text/html; charset=foo
      {
    ...
    while(*a) {
      ...
      do {
        ...
        token_st=a;                 // d�part token
        while((!is_space(*a)) && (*a) && (*a!=';') && (*a!='=')) a++;    // arr�ter si espace, point virgule
        token_end=a;
        while(is_space(*a)) a++;    // sauter espaces
        if (*a=='=') {    // name=value
          a++;
            ...
            if (strfield2(name,"domain")) {
              ...
              if (value_len < sizeof(cook_value) - 1 && name_len < sizeof(cook_name) - 1) {
                if (strnotempty(cook_name)==0) {          // noter premier: nom et valeur cookie
                  ...
                  a=start_loop;      // on devra recommencer � cette position
                  next=1;            // enregistrer
                }
              } else {
                cook_name[0] = 0;
...
// transforme le message statuscode en cha�ne
...
// identique au pr�c�dent, sauf que l'on donne adr+fil et non url compl�te
...
// variation sur un th�me...
// r�ceptionne uniquement un en-t�te (HEAD)
// retourne dans xx.adr l'adresse pointant sur le bloc de m�moire de l'en t�te
htsblk http_gethead(httrackp *opt,char* adr,char* fil) {
  T_SOC soc;
  htsblk retour;

  memset(&retour, 0, sizeof(htsblk));
  soc=http_xfopen(opt,1,0,1,NULL,adr,fil,&retour);  // HEAD, pas de traitement en-t�te

  if (soc!=INVALID_SOCKET) {
    http_fread(soc,&retour);    // r�ception en-t�te
#if HTS_DEBUG_CLOSESOCK
...
// oui ca ressemble vachement � xhttpget - en �tant sobre on peut voir LA diff�rence..


// lecture sur une socket ouverte, le header a d�ja �t� envoy� dans le cas de GET
// il ne reste plus qu'� lire les donn�es
...
// idem, sauf qu'ici on peut choisir la taille max de donn�es � recevoir
// SI bufl==0 alors le buffer est cens� �tre de 8kos, et on recoit par bloc de lignes
// en �liminant les cr (ex: header), arr�t si double-lf
// SI bufl==-1 alors le buffer est cens� �tre de 8kos, et on recoit ligne par ligne
// en �liminant les cr (ex: header), arr�t si double-lf
// Note: les +1 dans les malloc sont d�s � l'octet nul rajout� en fin de fichier
LLint http_xfread1(htsblk* r,int bufl) {
  ...
  if (bufl>0) {
    if (!r->is_write) {     // stocker en m�moire
      if (r->totalsize>0) {    // totalsize d�termin� ET ALLOUE
        if (r->adr==NULL) {
        ...
        if (r->adr != NULL) {
          ...
          */
          ...
          r->adr[r->size]='\0';    // caract�re NULL en fin au cas o� l'on traite des HTML
        }
    ...
    return -1;
  } else {    // r�ception d'un en-t�te octet par octet
    int count=256;
    int tot_nl=0;
    int lf_detected=0;
    int at_beginning=1;
      ...
      if (r->adr!=NULL) {
        if (r->size < 8190) {
          ...
          if (nl > 0) {
            ...
            if (*(r->adr+r->size) != 13) {   // sauter caract�res 13
              if (
                (*(r->adr+r->size) != 10)
                &&
                (*(r->adr+r->size) != 13)
...
// teste une adresse, et suit l'�ventuel chemin "moved"
// retourne 200 ou le code d'erreur (404=NOT FOUND, etc)
// copie dans loc la v�ritable adresse si celle-ci est diff�rente
...
// teste si une URL (validit�, header, taille)
// retourne 200 ou le code d'erreur (404=NOT FOUND, etc)
// en cas de moved xx, dans location
// abandonne d�sormais au bout de 30 secondes (aurevoir les sites
...
htsblk http_test(httrackp *opt,char* adr,char* fil,char* loc) {
  ...
  loc[0]='\0';
  memset(&retour, 0, sizeof(htsblk));    // effacer
  retour.location=loc;    // si non nul, contiendra l'adresse v�ritable en cas de moved xx

  //soc=http_fopen(adr,fil,&retour,NULL);  // ouvrir, + header

  // on ouvre en head, et on traite l'en t�te
  soc=http_xfopen(opt,1,0,1,NULL,adr,fil,&retour);  // ouvrir HEAD, + envoi header
  
  if (soc!=INVALID_SOCKET) {
    int e=0;
    // tant qu'on a des donn�es, et qu'on ne recoit pas deux LF, et que le timeout n'arrie pas
    do {
    ...
    if (e==1) {
      if (adr!=NULL) {
        ...
        if (strnotempty(rcvd)==0)
          ptr+=binput(retour.adr+ptr,rcvd,1024);    // "certains serveurs bugg�s envoient un \n au d�but" (RFC)
...
// Cr�e un lien (http) vers une adresse internet iadr
...
int newhttp(httrackp *opt,const char* _iadr,htsblk* retour,int port,int waitconnect) {  
  ...
  if (strcmp(_iadr,"file://") != 0) {           /* non fichier */
    ...
    // tester un �ventuel id:pass et virer id:pass@ si d�tect�
    ...
    if (port==-1) {
      ...
      if (a) {
        ...
        // adresse normale (port par d�faut par la suite)
    ...
    if (soc==INVALID_SOCKET) {
      ...
      return INVALID_SOCKET;                        // erreur cr�ation socket impossible
    }
    ...
    // A partir de maintenant, on peut envoyer et recevoir des donn�es
    // via le flot identifi� par soc (socket): write(soc,adr,taille) et 
    // read(soc,adr,taille)

  } else {    // on doit ouvrir un fichier local!
    // il sera g�r� de la m�me mani�re qu'une socket (c'est idem!)

    soc=LOCAL_SOCKET_ID;    // pseudo-socket locale..
    // soc sera remplac� lors d'un http_fopen() par un handle v�ritable!
...
int ident_url_absolute(const char* url,char* adr,char* fil) {
  ...
  if (!strfield(adr,"file:")) {      // PAS file://
    ...
    // p pointe sur le d�but de l'adresse, ex: www.truc.fr/sommaire/index.html
    ...
    // *( adr+( ((int) q) - ((int) p) ) )=0;  // faut arr�ter la fumette!
    // recopier chemin /pub/..
    if (q[0] != '/')    // page par d�faut (/)
      strcatbuff(fil,"/");
    strcatbuff(fil,q);
    // SECURITE:
    ...
    a=strchr(fil,'?');
    if (a) 
      *a='\0';      /* couper query (inutile pour file:// lors de la requ�te) */
  ...
  // nommer au besoin.. (non utilis� normalement)
  if (!strnotempty(fil))
...
// conversion en b,Kb,Mb, nombre et type s�par�s
// limite: 2.10^9.10^6B
...
// envoi de texte (en t�tes g�n�ralement) sur la socket soc
...
int finput(int fd,char* s,int max) {
  ...
  do {
    ...
    if (c!=0) {
      switch(c) {
      case 10: c=0; break;
      case 13: break;  // sauter ces caract�res
      default: s[j++]=c; break;
      }
    }
  }  while((c!=0) && (j<max-1));
...
int ishtml(httrackp *opt,const char* fil) {
  ...
  if (*a == '.') {  // a une extension
    ...
    return ret;
  } else return -2;   // ind�termin�, par exemple /truc
}
...
HTSEXT_API void x_escape_http(char* s,int mode) {
  while(*s) {
    ...
    else if (mode==3) {                   // �chapper que ce qui est n�cessaire
      test = (
                CHAR_SPECIAL(*s)
             || CHAR_XXAVOID(*s) );
    }
    else if (mode==30) {                   // �chapper que ce qui est n�cessaire
      test =
        ( *s != '/' && CHAR_RESERVED(*s) )
        || CHAR_DELIM(*s)
        || CHAR_UNWISE(*s)
...
// concat, concat�ne deux chaines et renvoi le r�sultat
// permet d'all�ger grandement le code
...
// remplacer un caract�re d'une cha�ne dans une autre
...
// caract�re espace, guillemets, CR, LF etc..
/* SECTION OPTIMISEE:
...
// types critiques qui ne doivent pas �tre chang�s car renvoy�s par des serveurs qui ne
...
int may_unknown(httrackp *opt,const char* st) {
  ...
  while(strnotempty(hts_mime_keep[j])) {
    if (strfield2(hts_mime_keep[j],st)) {      // trouv�
      return 1;
    }
    j++;
  }    
...
#endif
    //HTS_TOTAL_RECV_CHECK(size);         // Diminuer au besoin si trop de donn�es re�ues
#if HTS_USEOPENSSL
    if (SSL_is_available && r->ssl) {
...
// lock le cache dns pour tout op�ration d'ajout
// plus prudent quand plusieurs threads peuvent �crire dedans..
// -1: status? 0: lib�rer 1:locker
...
// routine pour le cache - retour optionnel � donner � chaque fois
// NULL: nom non encore test� dans le cache
...
t_hostent* _hts_ghbn(t_dnscache* cache,const char* iadr,t_hostent* retour) {
  ...
  for(;;) {
    if (strcmp(cache->iadr,iadr) == 0) {  // ok trouv�
      if (cache->host_length > 0) {  // entr�e valide
        ...
        ret = NULL;
        break;
      } else {                    // erreur dans le dns, d�ja v�rifi�
        if (retour->h_addr_list[0])
...
// tester si iadr a d�ja �t� test� (ou en cours de test)
// 0 non encore
// 1 ok
// 2 non pr�sent
...
#ifdef _WIN32
  if (inet_addr(iadr)!=INADDR_NONE)  // num�rique
#else
  if (inet_addr(iadr)!=(in_addr_t) -1 )  // num�rique
#endif
    return 1;
...
// cache dns interne � HTS // ** FREE A FAIRE sur la chaine
t_hostent* hts_gethostbyname(httrackp *opt,const char* _iadr, void* v_buffer) {
  ...
  // effacer structure de retour, cr�er nouvelle
  /*
  ...
  */
  ...
  if (hp) {
    if (hp->h_length>0)
      ...
      return NULL;    // entr�e erron�e (erreur DNS) dans le DNS
  } else {  // non pr�sent dans le cache dns, tester
    t_dnscache* c=cache;
...
#endif
    {
      ...
      } else {     // num�rique, convertir sans passer par le dns
    ...
    if (cache->n!=NULL) {
      ...
      cache->n->n=NULL;
      return hp;
    } else {  // on peut pas noter, mais on peut renvoyer le r�sultat
      return hp;
    }        
  }  // retour hp du cache
}
...
HTSEXT_API httrackp *hts_create_opt(void) {
  ...
  //  
  opt->travel=0;   // m�me adresse
  opt->depth=9999; // mirror total par d�faut
  opt->extdepth=0; // mais pas � l'ext�rieur
  opt->seeker=1;   // down 
  opt->urlmode=2;  // relatif par d�faut
  opt->debug=0;    // pas de d�bug en plus
  ...
  opt->timeout=120; // timeout par d�faut (2 minutes)
  ...
  StringCopy(opt->from, "");
  opt->savename_83=0;     // noms longs par d�faut
  ...
  opt->retry=2;           // 2 retry par d�faut
  opt->errpage=1;         // copier ou g�n�rer une page d'erreur en cas d'erreur (404 etc.)
  opt->check_type=1;      // v�rifier type si inconnu (cgi,asp..) SAUF / consid�r� comme html
  ...
  opt->includequery=1;    // include query-string par d�faut
  opt->mirror_first_page=0;  // pas mode mirror links
  opt->accept_cookie=1;   // g�rer les cookies
  opt->cookie=NULL;
  ...
  opt->parsedebug=0;      // pas de mode d�buggage
  opt->norecatch=0;       // ne pas reprendre les fichiers effac�s par l'utilisateur
  ...
  opt->maxlink=100000;    // 100,000 liens max par d�faut (400Kb)
  opt->maxfilter=200;     // 200 filtres max par d�faut
#else
  opt->maxlink=10000;     // 10,000 liens max par d�faut (40Kb)
  opt->maxfilter=50;      // 50 filtres max par d�faut
#endif
  opt->maxcache=1048576*32;  // a peu pr�s 32Mo en cache max -- OPTION NON PARAMETRABLE POUR L'INSTANT --
  //opt->maxcache_anticipate=256;  // maximum de liens � anticiper
...
static int __cdecl htsdefault_loop(t_hts_callbackarg *carg, httrackp *opt, lien_back* back,int back_max,int back_index,int lien_n,int lien_tot,int stat_time,hts_stat_struct* stats) {    // appel� � chaque boucle de HTTrack
  return 1;
}
